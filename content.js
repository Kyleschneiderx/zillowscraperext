...
class ZillowScraper {
  ...
  async scrapeMultiplePages(maxPages = 5, userTypeData = 'free', limitInfoData = null) {
  ...
  async scrollToBottom() {
    ...
    const resultsContainer = document.querySelector('[data-testid="search-page-list-container"]') ||
...
(async () => {
  ...
  setTimeout(async () => {
    ...
    if (pageNum) {
      ...
      try {
        const maxPages = sessionStorage.getItem('scrapeMaxPages') || 5;
